...
{
    if (_is_open && !mq_close(mqdes) && (!_need_to_rm || (_need_to_rm && !mq_unlink(MQ_NAME)))) 
    {
...
bool conn_t::conn_recv(void *buf, size_t count)
{
    ...
    if (mq_receive(mqdes, (char *)buf, count, nullptr) == -1)
    {
...
bool conn_t::conn_send(void *buf, size_t count)
{
    ...
    if (mq_send(mqdes, (char *)buf, count, 0) == -1) 
    {
