...
#include "../interfaces/set_optimistic.h"
...
{
    if (setType == SetType::OPTIMISTIC)
        return new SetOptimistic<int>();
...
std::string GetNameTest(TestType testType, SetType setType, DataType dataType)
{
    ...
    if (testType == TestType::TEST_READERS)
        ...
        tt = "Test Common";
    return (setType == SetType::LAZY ? "Lazy " : "OptimisticList ") + tt + (dataType == DataType::RANDOM ? " random" : " fixed");
