...
std::vector<int>& Server::split(const std::string &s, char delim, std::vector<int> &elems) {
...
std::vector<int> Server::split(const std::string &str, char delim) {
...
std::vector<int> Server::dateFromStr(std::string date) {
    std::vector<int> arr = split(date, '.');
    return arr;

}

bool Server::IsRealDate(int day, int month, int year) {
    if (year <= 0) {
        return false;
    }
    if (month <= 0 || month > 12) {
...
bool Server::MessFromDate(std::string date, Message &mes) {
